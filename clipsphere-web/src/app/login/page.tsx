...
export default function LoginPage() {
  ...
  // Single navigation point — fires when isAuthenticated flips to true.
  // window.location.href (hard nav) cannot be cancelled by React re-renders.
  useEffect(() => {
    ...
    <div className="flex min-h-[calc(100vh-160px)] items-center justify-center px-4 bg-gradient-to-br from-brand-500/5 via-transparent to-violet-500/5">
      <div className="glass glow-accent w-full max-w-md rounded-2xl p-8 animate-fade-in-up">
        ...
        <p className="mt-6 text-center text-sm text-text-muted">
          ...
          <Link href="/register" className="font-medium text-brand-400 hover:text-brand-300">Create one</Link>
        </p>
      </div>
    </div>
  );
